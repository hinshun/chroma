...
var Go = internal.Register(MustNewLexer(
	...
	Rules{
		"root": {
			...
			{`(func|map|chan)\b`, KeywordDeclaration, nil},
			{`(var|struct|type|interface|const|case|default)`, KeywordReserved, nil},
			{Words(``, `\b`, `break`, `select`, `defer`, `go`, `else`, `goto`, `switch`, `fallthrough`, `if`, `range`, `continue`, `for`, `return`), Keyword, nil},
